...
function Store({ children }) {
  ...
  function handleCategories(item) {
    if (item.name === "All") {
      ...
      let newItems = food_items.filter(
        (singleItem) => singleItem.food_category === item.name
      );
      setCate(newItems);
    }
  }
  ...
  function handleInput(e) {
    let newItem = food_items.filter((item) =>
      item.food_name.toLowerCase().includes(e.target.value)
    );
    setCate(newItem);
  }

  function handleBagItems(item) {
    const itemPresent = bagItem.find(preItem => preItem.id===item.id)
    ...
    if (itemPresent) {
      ...
      setBagItem([...bagItem, food]);
    }   
  }

  function handleDeleteFood(id) {
    let updatedBag = bagItem.filter((item) => item.id !== id);
    setBagItem(updatedBag);
  }

  function increaseQty(inc) {
    const updatedBagItems = bagItem.map((item) =>
      item.id === inc.id ? { ...item, qty: item.qty + 1 } : item
    );
    setBagItem(updatedBagItems)
  }
  function decreaseQty(inc) {
    const updatedBagItems = bagItem.map((item) =>
      item.id === inc.id ? { ...item, qty: item.qty - 1 } : item
    );
    setBagItem(updatedBagItems)
